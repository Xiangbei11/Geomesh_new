...
class Member(object):
    def __init__(self, **kwargs):
        ...
        self.options.declare('points_to_be_projected', types=np.ndarray, default=np.array([]))
        self.options.declare('projection_direction',types=np.ndarray, default=np.zeros((2,3)))
        self.options.declare('projected_point_ids', types=list,default=[])
...
if __name__ == '__main__':
    #import Member
    test = Member(name = 0, projs = 'test', dirts = np.array(([0,0,0])))
    print(test)