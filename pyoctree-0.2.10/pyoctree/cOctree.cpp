...
cOctree::cOctree(vector<vector<double> > _vertexCoords3D, vector<vector<int> > _polyConnectivity, vector<vector<int> > _quadlist)
{
    ...
    setupPolyList(); 
    if (quadlist.size()>0){//// Nings code   
        setupQuadList();//// Nings code
    }//// Nings code
...
void cOctree::setupQuadList()//// Nings code
{
    unsigned int i, j;
...
{
    return findBranchById(nodeId,root);// cout << nodeId;
}
...
//-------------------Changed by Ning's code start here: this is the definition of cquad---------------------------------------------------------------------------------
// ----------------------------------------  
cQuad::cQuad()
{
    label = 0;
...
// Reconnection cOctree::reconnect(cOctNode &node, vector<vector<double> > projpoints, vector<vector<double> > projdirts)
// {
//     unsigned int i, j, k, check, check0, check_v0, check_v1, check_e, check_i;
//     double eps_distance = 5e-5;//5e-4
//     vector<vector<double> > vertlist = vertexCoords3D;
//     vector<int> projlist;
//     vector<vector<int> > trilist = polyConnectivity;
//     vector<Intersection> proj;
...
//         int count = 0;
//         for (j=0;j<vertlist.size();j++){
//             if (distBetweenPoints(p0,vertlist[j])<eps_distance){
//                 vertlist[j] = p0;////
//                 projlisti.push_back(j);
//                 break;
//             }
...
//                     int count = 0;
//                     for (k=0;k<vertlist.size();k++){
//                         if (distBetweenPoints(intersect,vertlist[k])<eps_distance){
//                             vertlist[k] = intersect;////
//                             V.push_back(k);
//                             break;
//                         }
//                         if (k==vertlist.size()-1){
...
//             int count = 0;
//             for (k=0;k<vertlist.size();k++){
//                 if (distBetweenPoints(proj[i+1].p,vertlist[k])<eps_distance){
//                     vertlist[k] = proj[i+1].p;////
//                     projlisti.push_back(k);////projlisti ???V.push_back(k)
//                     //cout<<"V.push_back(k) "<<k<<'\n';
//                     break;
//                 }
//                 if (k==vertlist.size()-1){
//                     count = 1;
...
//         }
//         projlist.insert(projlist.end(),projlisti.begin(),projlisti.end());
//         //cout<<"update trilist "<<i<<" projlist.size"<<projlist.size()<<'\n';
        
//         //cout<<"update trilist "<<i<<" vertlist.size"<<vertlist.size()<<'\n';
//         // update trilist
//         cout<<"update trilist "<<i<<" projlisti.size"<<projlisti.size()<<'\n';
//         for (j=0;j<projlisti.size();j++){
//             vector<vector<int> > tripush;
//             vector<double> Pi = vertlist[projlisti[j]];
...
//                 if ((area1+area2+area3-area)<1e-5){// check area_------(area1+area2+area3-area)<1e-5
//                     // cout<<area1<<'\n';
//                     // cout<<area2<<'\n';
//                     // cout<<area3<<'\n';
//                     // cout<<area<<'\n';
//                     // projection locates in the tri 4807
//                     check0 = 1;
//                     if((abs(area1-area)<1e-5) || (abs(area2-area)<1e-5) || (abs(area3-area)<1e-5)){
                        
//                         //cout<<"on vert "<<k<<'\n';
//                         check_v0 = check_v0 + 1;
//                         if((projlisti[j] != trilist[k][0]) && (projlisti[j] != trilist[k][1]) && (projlisti[j] != trilist[k][2])){
//                             //cout<<k<<" WARN3 "<<" j:"<<j<<" "<<projlisti[j]<<": "<<trilist[k][0]<<" "<<trilist[k][1]<<" "<<trilist[k][2]<<'\n';
//                             //check_vector.push_back(projlisti[j]);
//                             check_v1 = check_v1 + 1;
//                         }
//                         // projection locates on the vertices
...
//             if((check0 == 1) && (check_v0 != 0) && (check_v1 == check_v0) && (check_e == 0) && (check_i == 0)){
//                 cout<<"WARN123 check "<<projlisti[j]<<'\n';
//                 double temp_distance = 1000.;
//                 int temp_k = 0;
//                 for (k=0;k<vertlist.size();k++){
//                     if (k == projlisti[j]){
//                         ;
//                     }else if (distBetweenPoints(vertlist[projlisti[j]],vertlist[k])<temp_distance){
//                         temp_k = k;
//                         temp_distance = distBetweenPoints(vertlist[projlisti[j]],vertlist[k]);
//                     }
//                 }
//                 vertlist[temp_k] = proj[i].p;
//             }
