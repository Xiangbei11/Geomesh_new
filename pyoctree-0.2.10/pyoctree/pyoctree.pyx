...
# most codes are directly from pyoctree, 
# codes that look dumb and have less comments are usually done by Ning/Xiangbei
# generally speaking, the only thing that is done in pyx file is 
# transformation of data types bewtween python and cpp variables, 
# in our code, no calculation is done in here
...
cdef extern from "cOctree.h":
    ...
    cdef cppclass cOctree:
        ...
        #vector[bint] findRayIntersects(vector[cLine] &rayList) ### Changed by Ning
        ...
        # ------------ Changed by Xiangbei
        vector[Intersection] ProjectPoints(cOctNode &node, vector[vector[double]] projpoints, vector[vector[double]] projdirts);
        Reconnection ProjectandReconnect(cOctNode &node, vector[vector[double]] projpoints, vector[vector[double]] projdirts);
        # ------------
...
# ---------------------------------------- Changed by Xiangbei 
    ...
    def ProjectandReconnect(self, np.ndarray[float,ndim =2] _projpoints, np.ndarray[float,ndim=2] _projdirts):
        ...
        cdef Reconnection reconnection = self.thisptr.ProjectandReconnect(self.thisptr.root, projpoints, projdirts)
        ...
        return pyreconnection

    def ProjectPoints(self, np.ndarray[float,ndim =2] _projpoints, np.ndarray[float,ndim=2] _projdirts):
        cdef vector[vector[double]] projpoints
        projpoints.resize(_projpoints.shape[0])
        for i in range(_projpoints.shape[0]):
            projpoints[i].resize(3)
        ...
        cdef vector[Intersection] proj = self.thisptr.ProjectPoints(self.thisptr.root, projpoints, projdirts)
        # i = np.shape(projpoints)[0]
        # print(projpoints[i-1][0],projpoints[i-1][0],projpoints[i-1][0]), projpoints[_projpoints.shape[0]][1], projpoints[_projpoints.shape[0]][2]

        cdef vector[int] proj_triLabel = range(_projpoints.shape[0])
        for i in range(_projpoints.shape[0]):
...
# -------------------- Changed by Ning 
    ...
    def calculateavgedgesize(self):
        cdef double size = self.thisptr.getavgpolysize(self.thisptr.root)
        return size
    ...
    def findmindist(self, np.ndarray[float,ndim=1] _projpoint):
        ...
        # print(distandposition.distance)
    ...
    def reconnect(self, np.ndarray[float,ndim =2] _projpoints, np.ndarray[float,ndim=2] _projdirts):
        cdef vector[vector[double]] projpoints
        projpoints.resize(_projpoints.shape[0])
        for i in range(_projpoints.shape[0]):
            projpoints[i].resize(3)
