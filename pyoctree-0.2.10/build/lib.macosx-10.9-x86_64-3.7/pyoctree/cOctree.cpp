...
{   
    if (node.isLeafNode()){
        ...
        Intersection mindistandpos = findmindist(node.branches[minbranchnum], projpoint); 
...
Projection cOctree::quadproject(vector<double> p0, int index)
{
    ...
    // vector<double> v0 = vertexCoords3D[quadlist[min_i_local][0]];
    // vector<double> v1 = vertexCoords3D[quadlist[min_i_local][1]];
    // vector<double> v2 = vertexCoords3D[quadlist[min_i_local][2]];
    // vector<double> v3 = vertexCoords3D[quadlist[min_i_local][3]];
    // x[0] = min_u_local;
    // x[1] = min_v_local;
    // vector<double> pu(3,0.0);
    // vector<double> pv(3,0.0);
    // vector<double> puu(3,0.0);
    // vector<double> pvv(3,0.0);
    // vector<double> puv(3,0.0);
    // vector<double> f(3,0.0);
    // vector<double> p(3,0.0);
    // for (l=0;l<3;l++){
    //     p[l] = 0.5*(1-x[0])*0.5*(1-x[1])*v0[l] + 0.5*(1+x[0])*0.5*(1-x[1])*v1[l] + 0.5*(1+x[0])*0.5*(1+x[1])*v2[l] + 0.5*(1-x[0])*0.5*(1+x[1])*v3[l];
    //     pu[l] = -0.25*(1-x[1])*v0[l] + 0.25*(1-x[1])*v1[l] + 0.25*(1+x[1])*v2[l] - 0.25*(1+x[1])*v3[l];
    //     pv[l] = -0.25*(1-x[0])*v0[l] - 0.25*(1+x[0])*v1[l] + 0.25*(1+x[0])*v2[l] + 0.25*(1-x[0])*v3[l];
    //     puv[l] = 0.25*v0[l] - 0.25*v1[l] + 0.25*v2[l] - 0.25*v3[l];
    // }
    // f = vectSubtract(p,p0);
    // vector<vector<double> > dRdx(2,vector<double>(3,0.0));
    // vector<vector<double> > dRdy(2,vector<double>(2,0.0));
    ...
    // vector<vector<double> > dfdx(3,vector<double>(3,0.0));
    // vector<vector<double> > invdRdy(2,vector<double>(2,0.0));
    // if (((min_u_local==-1)||(min_u_local==1))&&((min_v_local==-1)||(min_v_local==1))){// vertex
        
    // }else if ((min_u_local==-1)||(min_u_local==1)){
    //     dRdx[1][0] = -pv[0]; dRdx[1][1] = -pv[1]; dRdx[1][2] = -pv[2];
    //     dRdy[0][0] = 1; dRdy[1][0] = dotProduct(puv,f) + dotProduct(pu,pv); dRdy[1][1] = dotProduct(pvv,f) + dotProduct(pv,pv);
    //     dFdy[0][0] = pu[0]; dFdy[1][0] = pu[1]; dFdy[2][0] = pu[2]; dFdy[0][1] = pv[0]; dFdy[1][1] = pv[1]; dFdy[2][1] = pv[2]; 
    //     invdRdy = vectInverse(dRdy); dydx = vectMultiply(invdRdy,dRdx); dydx = vectneg(dydx);
    //     dfdx = vectMultiply(dFdy,dydx);
    // }else if ((min_v_local==-1)||(min_v_local==1)){
    //     dRdx[0][0] = -pu[0]; dRdx[0][1] = -pu[1]; dRdx[0][2] = -pu[2];
    //     dRdy[0][0] = dotProduct(puu,f) + dotProduct(pu,pu); dRdy[0][1] = dotProduct(puv,f) + dotProduct(pu,pv); dRdy[1][1] = 1;
    //     dFdy[0][0] = pu[0]; dFdy[1][0] = pu[1]; dFdy[2][0] = pu[2]; dFdy[0][1] = pv[0]; dFdy[1][1] = pv[1]; dFdy[2][1] = pv[2]; 
    //     invdRdy = vectInverse(dRdy); dydx = vectMultiply(invdRdy,dRdx); dydx = vectneg(dydx);
    //     dfdx = vectMultiply(dFdy,dydx);
    // }else{
    //     dRdx[0][0] = -pu[0]; dRdx[0][1] = -pu[1]; dRdx[0][2] = -pu[2]; dRdx[1][0] = -pv[0]; dRdx[1][1] = -pv[1]; dRdx[1][2] = -pv[2];
    //     dRdy[0][0] = dotProduct(puu,f) + dotProduct(pu,pu); dRdy[0][1] = dotProduct(puv,f) + dotProduct(pu,pv);
    //     dRdy[1][0] = dotProduct(puv,f) + dotProduct(pu,pv); dRdy[1][1] = dotProduct(pvv,f) + dotProduct(pv,pv);
    //     dFdy[0][0] = pu[0]; dFdy[1][0] = pu[1]; dFdy[2][0] = pu[2]; dFdy[0][1] = pv[0]; dFdy[1][1] = pv[1]; dFdy[2][1] = pv[2]; 
    //     invdRdy = vectInverse(dRdy); dydx = vectMultiply(invdRdy,dRdx); dydx = vectneg(dydx);
    //     dfdx = vectMultiply(dFdy,dydx);
    // }
...
Reconnection cOctree::reconnect(cOctNode &node, vector<vector<double> > projpoints, vector<vector<double> > projdirts)
{
    ...
    // vector<double> e0(3,0.0);
    // vector<double> e1(3,0.0);
    // vector<double> mid(3,0.0);
    // vector<double> p0(3,0.0);
    // vector<double> p1(3,0.0);
    // vector<double> p0m(3,0.0);
    // vector<double> p1m(3,0.0);
    ...
    // double temp2 = 0.0;
    // double temp3 = 0.0;
    // vector<double> intersect(3,0.0);
    // vector<double> ie0(3,0.0);
    // vector<double> ie1(3,0.0);
    // vector<double> midp(3,0.0);
    // vector<double> midpp1(3,0.0);
    // vector<double> p2(3,0.0);
    // vector<double> p3(3,0.0);
    // double area = 0.0, area0 = 0.0, area1 = 0.0, area2 = 0.0, area3 = 0.0;
    // vector<double> n(3,0.0), n0(3,0.0), n1(3,0.0), n2(3,0.0), n3(3,0.0);
    // vector<double> p0i(3,0.0), p1i(3,0.0), p2i(3,0.0), p3i(3,0.0);
    // vector<vector<double> > inti;
    // vector<int> V;
    // for (i=0;i<projpoints.size()-1;i++){// loop through all the projected points
    //     inti.clear();
    //     V.clear();
    //     mid = midPoint(projpoints[i],projpoints[i+1]);// find the midpoint of two projection points
    //     p0 = proj[i].p; p1 = proj[i+1].p;// find the cooresponding projected points 
    //     vertlist.push_back(p0); projlist.push_back(vertlist.size()-1);// first add the first point to vertlist
    //     p0m = vectSubtract(mid,p0); p1m = vectSubtract(mid,p1);// vectors pointing from the projected point to the midpoint
    //     normal = crossProduct(p0m,p1m); normal = normalize(normal);// find the normal vector of the plane on which we have the midpoint and two projected points
    //     midp = midPoint(p0,p1); midpp1 = vectSubtract(p1,midp);// midpoint of projected points and vector point from midp to p1
    //     p2 = vectAdd(mid,midpp1); p3 = vectSubtract(mid,midpp1);// find the cooresponding p2 and p3 of the parallelogram
    //     for (j=0;j<edges.size();j++){// loop through all the edges to find the intersection
    //         e0 = vertexCoords3D[edges[j][0]];// the starting vertex of the edge
    //         e1 = vertexCoords3D[edges[j][1]];// the ending vertex of the edge
    //         l = vectSubtract(e0,e1);// vector from e1 to e0
    //         temp1 = vectSubtract(p0,e0);
    //         temp2 = dotProduct(temp1,normal);
    //         temp3 = dotProduct(l,normal);
    //         d = temp2/temp3;
    //         intersect = vectAdd(e0,l,d);// find the intersection point between the plane and the line
    //         ie0 = vectSubtract(e0,intersect); ie1 = vectSubtract(e1,intersect);
    //         if (dotProduct(ie0,ie1)<0){// see if edge intersects with the plane 
    //             n = crossProduct(p0m,p1m); area  = sqrt(dotProduct(n,n));
    //             p0i = vectSubtract(intersect,p0); p1i = vectSubtract(intersect,p1); p2i = vectSubtract(intersect,p2); p3i = vectSubtract(intersect,p3);
    //             n0 = crossProduct(p0i,p1i); area0  = 0.5 * sqrt(dotProduct(n0,n0));
    //             n1 = crossProduct(p1i,p2i); area1  = 0.5 * sqrt(dotProduct(n1,n1));
    //             n2 = crossProduct(p2i,p3i); area2  = 0.5 * sqrt(dotProduct(n2,n2));
    //             n3 = crossProduct(p3i,p0i); area3  = 0.5 * sqrt(dotProduct(n3,n3));
    //             if(area0+area1+area2+area3-area<1e-5){// see if the intersection point locates in the parallelogram
    //                 inti.push_back(intersect);// save intersection point
    //                 vertlist.push_back(intersect);// save to vertlist
    //                 V.push_back(vertlist.size()-1);// save index of intersection point
    //             }
    //         }
    //     }
    //     vector<double> dists(inti.size(),0.0);
    //     for (k=0;k<inti.size();k++){
    //         dists[k] = distBetweenPoints(inti[k],p0);// calculate the distances between the intersection points and projected point
    //     }
    //     vector<double> reference; reference.assign(dists.begin(), dists.end());// save the distances as reference
        
    //     sort(dists.begin(),dists.end());// sort distance in the increasing order
        
    //     for (k=0;k<inti.size();k++){// sort index by distances 
    //         for (m=0;m<inti.size();m++){
    //             if (reference[m]==dists[k]){
    //                 projlist.push_back(V[m]);
    //             }
    //         }
    //     }
    //     if (i==projpoints.size()-2){// at the second last point, add the last point
    ...
    //     }
    // }
    // projlist.push_back(vertlist.size()-1);// at the second last projected point, add the last projected point
    // vector<double> Pi(3,0.0);
    // vector<double> P1(3,0.0);
    // vector<double> P2(3,0.0);
    // vector<double> P3(3,0.0);
    // vector<double> PiP1(3,0.0);
    // vector<double> PiP2(3,0.0);
    // vector<double> PiP3(3,0.0);
    // vector<double> P1P2(3,0.0);
    // vector<double> P1P3(3,0.0);
    // vector<double> cross1(3,0.0), cross2(3,0.0), cross3(3,0.0), cross(3,0.0); 
    // vector<vector<int> > tripush;

    
    // for (p=0;p<projlist.size();p++){// loop through all the projected points
    //     tripush.clear();
    //     Pi = vertlist[projlist[p]];
    //     for (o=0;o<trilist.size();o++){// check the projected point has intersection with which tri
    //         P1=vertlist[trilist[o][0]];
    //         P2=vertlist[trilist[o][1]];
    //         P3=vertlist[trilist[o][2]];    
    //         PiP1=vectSubtract(Pi,P1);
    //         PiP2=vectSubtract(Pi,P2);
    //         PiP3=vectSubtract(Pi,P3);
    //         P1P2=vectSubtract(P1,P2);
    ...
    //         if (area1+area2+area3-area<1e-5){// check area
    //             // projection locates in the tri
    //             if ((area1<1e-5)||(area2<1e-5)||(area3<1e-5)){
    //                 // projection locates on the edges
    //                 if (area1<1e-5){
    //                     int tri1list[] = {trilist[o][0],projlist[p],trilist[o][2]}; vector<int> tri1(tri1list, tri1list + sizeof(tri1list) / sizeof(int) );
    //                     int tri2list[] = {projlist[p],trilist[o][1],trilist[o][2]}; vector<int> tri2(tri2list, tri2list + sizeof(tri2list) / sizeof(int) );
    //                     tripush.push_back(tri1);tripush.push_back(tri2);
    //                 }else if (area2<1e-5){
    //                     int tri1list[] = {trilist[o][1],projlist[p],trilist[o][0]}; vector<int> tri1(tri1list, tri1list + sizeof(tri1list) / sizeof(int) );
    //                     int tri2list[] = {projlist[p],trilist[o][2],trilist[o][0]}; vector<int> tri2(tri2list, tri2list + sizeof(tri2list) / sizeof(int) );
    //                     tripush.push_back(tri1);tripush.push_back(tri2);
    //                 }else{
    //                     int tri1list[] = {trilist[o][2],projlist[p],trilist[o][1]}; vector<int> tri1(tri1list, tri1list + sizeof(tri1list) / sizeof(int) );
    //                     int tri2list[] = {projlist[p],trilist[o][0],trilist[o][1]}; vector<int> tri2(tri2list, tri2list + sizeof(tri2list) / sizeof(int) );
    //                     tripush.push_back(tri1);tripush.push_back(tri2);
    //                 }
    //                 trilist.erase(trilist.begin()+o);

    //             // }else if((area1-area<1e-5) || (area2-area<1e-5) || (area3-area<1e-5)){
    //             //     if (area1-area<1e-5){
    //             //         projlist[p] = trilist[o][0];
    //             //     }else if (area2-area<1e-5){
    //             //         projlist[p] = trilist[o][1];
    //             //     }else{
    //             //         projlist[p] = trilist[o][2];
    //             //     }
    //             }else{
    //                 // projection locates on the interior
    //                 int tri1list[] = {trilist[o][0],projlist[p],trilist[o][2]}; vector<int> tri1(tri1list, tri1list + sizeof(tri1list) / sizeof(int) );
    //                 int tri2list[] = {trilist[o][1],projlist[p],trilist[o][0]}; vector<int> tri2(tri2list, tri2list + sizeof(tri2list) / sizeof(int) );
    //                 int tri3list[] = {trilist[o][2],projlist[p],trilist[o][1]}; vector<int> tri3(tri3list, tri3list + sizeof(tri3list) / sizeof(int) );
    //                 trilist.push_back(tri1);trilist.push_back(tri2);trilist.push_back(tri3);
    //                 trilist.erase(trilist.begin()+o);
    //                 break;
    //             }
    //         }
    //     }
    //     for (q=0;q<tripush.size();q++){
    //         trilist.push_back(tripush[q]);
    //     }
    // }


//------------------------------------------------------------------------------------------------------------------------
    // edges = countedges(trilist);// update edge list
    // cout<<"num_edges:"<<edges.size()<<'\n';
    
   
    // for (i=0;i<projlist.size()-1;i++){// loop through projected points
    //     vector<double> a = vertlist[projlist[i]];
    //     vector<double> b = vertlist[projlist[i+1]];
    //     inti.clear();
    //     V.clear();
    //     for (j=0;j<edges.size();j++){// loop through updated edges
    //         vector<double> c = vertlist[edges[j][0]];
    //         vector<double> d = vertlist[edges[j][1]];
    //         vector<double> T1 = vectSubtract(b,a);
    //         vector<double> M1 = crossProduct(a,b);
    //         vector<double> T2 = vectSubtract(d,c);
    //         vector<double> M2 = crossProduct(c,d);
    //         if (abs(dotProduct(T1,M2)+dotProduct(T2,M1))<1e-10){// check intersecion between the edge and the line segment connecting a and b
    //             vector<double> ba = vectSubtract(a,b);
    //             vector<double> dc = vectSubtract(c,d);
    //             vector<double> abcdn = crossProduct(ba,dc);
    //             vector<double> cdn = crossProduct(dc,abcdn);
    //             vector<double> abn = crossProduct(ba,abcdn);
    //             double a1 = dotProduct(cdn,b);
    //             double a2 = dotProduct(cdn,ba);
    //             double B = dotProduct(cdn,c);
    //             double t1 = (B-a1)/a2;// t1 and t2 are the location of intersection on the edge and line segment
    //             a1 = dotProduct(abn,d);
    //             a2 = dotProduct(abn,dc);
    //             B = dotProduct(abn,a);
    //             double t2 = (B-a1)/a2;
    //             // cout<<"\nt1:"<<t1<<'\n';
    //             // cout<<"t2:"<<t2<<'\n';
    //             // cout<<edges[j][0]<<'\n';
    //             // cout<<edges[j][1]<<'\n';
    //             if ((1e-5<t1)&&(t1<1-1e-5)&&(1e-5<t2)&&(t2<1-1e-5)){// t1 and t2 have to stay between 0 and 1 to make an intersection
                    
    //                 // cout<<"i:"<<i<<'\n';
    //                 // for (unsigned int z=0; z<3; z++){
    //                 //     cout<<vertlist[edges[j][1]][z];
    //                 // }
    //                 Pi = vectAdd(b,ba,t1);// compute the intersection point
    //                 vertlist.push_back(Pi);// add intersectiont points
    //                 V.push_back(vertlist.size()-1);
    //                 inti.push_back(Pi);
    //                 int index = vertlist.size()-1;
    //                 tripush.clear();
    //                 for (o=0;o<trilist.size();o++){// loop through triangles to find out which trianlge the intersection point is in (usually should be 2)
    //                     P1=vertlist[trilist[o][0]];
    //                     P2=vertlist[trilist[o][1]];
    //                     P3=vertlist[trilist[o][2]];    
    //                     PiP1=vectSubtract(Pi,P1);
    ...
    //                         // projection locates in the tri
    //                         if ((area1<1e-8)||(area2<1e-8)||(area3<1e-8)){
    //                             // projection locates on the edges
    //                             if (area1<1e-5){
    //                                 int tri1list[] = {trilist[o][0],index,trilist[o][2]}; vector<int> tri1(tri1list, tri1list + sizeof(tri1list) / sizeof(int) );
    //                                 int tri2list[] = {index,trilist[o][1],trilist[o][2]}; vector<int> tri2(tri2list, tri2list + sizeof(tri2list) / sizeof(int) );
    //                                 tripush.push_back(tri1);tripush.push_back(tri2);
    //                             }else if (area2<1e-5){
    //                                 int tri1list[] = {trilist[o][1],index,trilist[o][0]}; vector<int> tri1(tri1list, tri1list + sizeof(tri1list) / sizeof(int) );
    //                                 int tri2list[] = {index,trilist[o][2],trilist[o][0]}; vector<int> tri2(tri2list, tri2list + sizeof(tri2list) / sizeof(int) );
    //                                 tripush.push_back(tri1);tripush.push_back(tri2);
    //                             }else{
    //                                 int tri1list[] = {trilist[o][2],index,trilist[o][1]}; vector<int> tri1(tri1list, tri1list + sizeof(tri1list) / sizeof(int) );
    //                                 int tri2list[] = {index,trilist[o][0],trilist[o][1]}; vector<int> tri2(tri2list, tri2list + sizeof(tri2list) / sizeof(int) );
    //                                 tripush.push_back(tri1);tripush.push_back(tri2);
    //                             }
    //                             trilist.erase(trilist.begin()+o);
    //                         }else{
    ...
    //                 vector<double> dists(inti.size(),0.0);
    //                 for (k=0;k<inti.size();k++){
    //                     dists[k] = distBetweenPoints(inti[k],a);
    //                 }
    //                 vector<double> reference; reference.assign(dists.begin(), dists.end());
    ...
    //                     for (m=0;m<inti.size();m++){
    //                         if (reference[m]==dists[k]){
    //                             projlist.insert(projlist.begin()+i+k+1,V[m]);
    //                             // i=i+1;
    //                         }
    //                     }
    //                 }
