...
class DesignGeometry:
    ...
    def project_points(self, points_to_be_projected, projection_targets_names=[], projection_direction=np.array([0., 0., 0.]), offset=np.array([0., 0., 0.]), plot=False):
        ...
        for surf in projection_targets: 
            ...
            print('surface control points: ', surf.control_points.shape)
            print('Projection target shape: ', projection_targets[0].shape[0]*projection_targets[0].shape[0]*3)
            if  np.prod(surf.control_points.shape) < 675:
                continue
            else:
