...
cdef extern from "cmeshopt.h":
    ...
    cdef cppclass opt:
        opt(vector[vector[double]] _vertexCoords, vector[vector[int]] _triConnectivity, vector[vector[int]] _quadConnectivity, double _w1, double _w2, double _w3, double _w4)
...
cdef class Pymeshopt:
    ...
    def __cinit__(self, np.ndarray[float,ndim=2] _vertexCoords, np.ndarray[int,ndim=2] _triConnectivity, np.ndarray[int,ndim=2] _quadConnectivity, float _w1, float _w2, float _w3, float _w4):
        ...
        cdef double w3 = _w3
        cdef double w4 = _w4
        self.thisopt = new opt(vertexCoords, triConnectivity, quadConnectivity, w1, w2, w3, w4)
    ...
    def ubcon(self):
        ...
        #print('Define pyubcon')
        #print(np.shape(ubcon))
        #print(ubcon.size())
        #print(pyubcon.size())
        #print(np.shape(pyubcon))
        for i in range(ubcon.size()):
            for j in range(ubcon[0].size()):
                pyubcon[i,j] = ubcon[i][j]
        #free(ubcon)
        #print('Redefine pyubcon')      
        return pyubcon
