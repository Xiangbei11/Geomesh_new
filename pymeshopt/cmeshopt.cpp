...
opt::opt(vector<vector<double> > _vertexCoords, vector<vector<int> > _triConnectivity, vector<vector<int> > _quadConnectivity, double _w1, double _w2, double _w3, double _w4)
{
...
vector<double> opt::splitquad(vector<vector<double> > vertices)
{
    ...
    options[10] = w1*(pow(angleRatio(e01,e03,theta1),p) + pow(angleRatio(e10,e12,theta1),p)
               ...
               //deleted by Xiangbei + w3*(abs(dotProduct(cross,e01n)) + abs(dotProduct(cross,e12n)) + abs(dotProduct(cross,e23n)) + abs(dotProduct(cross,e30n)))/4;
    return options;
}
...
{
    // fixedvert is an array of points, if an edge from the mesh has the same indices as two neighouring points from the fixedvert, then the edge is fixed
    // for instance, edge (10,100) is fixed if the fixedvert is (0,2,3,4,5,100,10,324,4324) since 10 and 100 are neighbouring points in fixedvert
...
vector<double> opt::mergetriangles()
{
    ...
    for (i=0;i<indices.triindex.size();i++){
        ...
        energyafter = w4*(w1*(pow(angle0,p) + pow(angle1,p) + pow(angle[0],p) + pow(angle[1],p))/4
                            + w2*(pow(vectNorm(e01)/quadLavg,p) + pow(vectNorm(e12)/quadLavg,p) + pow(vectNorm(e23)/quadLavg,p) + pow(vectNorm(e30)/quadLavg,p))/4
                            + w3*(abs(dotProduct(cross,e01n)) + abs(dotProduct(cross,e12n)) + abs(dotProduct(cross,e23n)) + abs(dotProduct(cross,e30n)))/4);
        //cout<<"energyafter*"<<w4<<"\n";
...
{
    unsigned int i, j;
    //cout<<"START inequncon"<<'\n';
    vector<vector<int> > edges = countedges();
    //cout<<"countedges"<<'\n';
    vector<vector<int> > A_uq(triConnectivity.size(), vector<int> (edges.size(),0.0));
    //int A_uq_test[triConnectivity.size()][edges.size()]
    //cout<<triConnectivity.size()<<'\n';
    //cout<<edges.size()<<'\n';
    //cout<<"Define A_uq"<<'\n';
    ...
    for (i=0;i<vertlist.size();i++){
        ...
        for (j=0;j<quadlist.size();j++){
            for (k=0;k<4;k++){
                ...
                if (((int) i)==quadlist[j][k]){// see which point it is in the quad
                    check_point += 1;
                    if (k==0){
        ...
        if (isnan(normali[0]) || isnan(normali[1]) || isnan(normali[2])){
            //cout<<"NAN: "<<i<<'\n';
            //check_point -= 1;
            //cout<<"check_point: "<<check_point<<'\n';
        }
        //cout<<"check_point: "<<check_point<<'\n';
        if (check_point == 0){
            cout<<"This point doesn't belong to any tri or quad: "<<i<<'\n';
            //normali[0] = 1;
            //normali[1] = 1;
            //normali[2] = 1;
        }
